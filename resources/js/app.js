...
$(".btn-value-order").click(function () {
    ...
    $(".modal-body #id_voucher").text(
        value.couponCode + "/" + value.moneyDiscount.toLocaleString("vi-VN")+'đ'
    );
    $(".modal-body #id_price_vanchuyen").text(value.shipFee.toLocaleString("vi-VN")+'đ');
    $(".modal-body #id_price_total").text(value.calcTotalMoney.toLocaleString("vi-VN")+'đ');
    ...
    value.products.forEach(function (product) {
        ...
        $(".modal-body #products-list-order").append(
            `<tr>
            <td class="overflow-hidden text-start py-2">${product.productName}</td>
  ...
  $(".modal-body #price_tamtinh").text(total_tamtinh.toLocaleString('vi-VN')+'đ');
});
